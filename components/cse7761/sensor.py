...
CONF_DEBUG_SENSOR_ID = "debug_sensor_id"
...
#                state_class=STATE_CLASS_MEASUREMENT,
#            ),
            cv.Optional(CONF_DEBUG_SENSOR_ID): cv.use_id(text_sensor.TextSensor), # Validation de l'ID du text_sensor
        }
    )
...
#        CONF_ACTIVE_POWER_2,
    ...
    if debug_sensor_config := config.get(CONF_DEBUG_SENSOR_ID):
        debug_sensor = await cg.get_variable(debug_sensor_config)
        cg.add(var.set_debug_text_sensor(debug_sensor))
